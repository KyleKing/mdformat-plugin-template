"""Markdown-it plugins for eb_plugin_example."""

from markdown_it.rules_block import StateBlock

from mdformat_eb_plugin_example._synced.admon_factories import (
    AdmonitionData,
...
def format_eb_admon_markup(
    state: StateBlock,
    start_line: int,
    admonition: AdmonitionData,
) -> None:
    """Format markup for eb_admon."""
    ...
    with new_token(state, PREFIX, "div") as token:
        ...
        token.block = True
        token.attrs = {"class": " ".join(["eb-admon", f"eb-admon-{tag}"])}
        token.meta = {"tag": tag}
        ...
        with new_token(state, f"{PREFIX}_title", "p") as tkn_title:
            ...
            tkn_title.attrs = {"class": "eb-admon-title"}
            tkn_title.map = [start_line, start_line + 1]

            tkn_inline = state.push("inline", "", 0)
            tkn_inline.content = title or tag.title()
...
def eb_admon_logic(
    ...
    """Parse eb_admon-style admonitions.
    ...
    if isinstance(result, AdmonitionData):
        format_eb_admon_markup(state, start_line, admonition=result)
        return True
    return result


eb_plugin_example_plugin = admon_plugin_factory(PREFIX, eb_admon_logic)


__all__ = ["eb_plugin_example_plugin"]
